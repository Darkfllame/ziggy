...
pub fn link(sdk: *Sdk, exe: *LibExeObjStep, linkage: LibExeObjStep.Linkage) void {
    ...
    if (target.os.tag == .linux and !is_native) {
        ...
        if (target.abi == .msvc) {
            ...
            const lib_path2 = std.fs.path.join(b.allocator, &[_][]const u8{
                sdk_paths.libs,
                "libSDL2_image.a",
            }) catch @panic("out of memory");
            const lib_path3 = std.fs.path.join(b.allocator, &[_][]const u8{
                sdk_paths.libs,
                "libSDL2_mixer.a",
            }) catch @panic("out of memory");

            exe.addObjectFile(.{ .path = lib_path });
            exe.addObjectFile(.{ .path = lib_path2 });
            exe.addObjectFile(.{ .path = lib_path3 });
