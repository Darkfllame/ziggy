// Thanks to JakubSzark from https://github.com/JakubSzark/zig-string for this precious code :)
...
pub const String = struct {
    ...
    pub fn format(self: String, comptime _: []const u8, _: std.fmt.FormatOptions, stream: anytype) !void {
        try stream.print("\"{?s}\"", .{self.str()});
    }
    pub fn jsonStringify(self: String, stream: anytype) !void {
        try stream.print("\"{?s}\"", .{self.str()});
    }
    pub fn jsonParse(allocator: std.mem.Allocator, source: anytype, _: std.json.ParseOptions) !String {
        return init_with_contents(allocator, (try source.next()).string) catch init(allocator);
    }
    ...
    pub fn startWith(self: *String, pattern: []const u8) bool {
        var result = false;
        for (pattern, 0..) |c, i| result = result and self.charAt(i).?[0] == c;
        return result;
    }
    ...
    /// Returns the hash function for this string
    /// From Darkfllame
    pub fn getHash(self: String) u64 {
        return hashOf(self.str());
    }

    pub fn hashOf(s: []const u8) u64 {
        var hash: u64 = 5381;
        for (s) |c| {
            hash = ((hash << 5) +% 5) +% c;
        }
